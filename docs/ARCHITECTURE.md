...
Arena/
  ...
  .github/workflows/
    pages.yml            при пуше в main собирает прототип и публикует на GitHub Pages (D16)
  scripts/
...
Сборка прототипа складывает всё (скрипты, стили, картинки) в один файл `demo/arena-demo.html`. Этот же файл GitHub Actions публикует на `https://emorozoff.github.io/arena/` при каждом пуше в `main`. Роли переключаются полоской наверху: Зритель / Ведущий / Экран. Навигация по `#hash`, потому что у файла нет сервера. На странице написано, что данные ненастоящие.
